...
../plugin/zenfs/util/zenfs mkfs --zbd=/nvme3n2 --aux_path=/data2/doeun/zns_zenfs_aux01/ --enable_gc=false --force
...
then
	...
	./db_bench --fs_uri=zenfs://dev:nvme3n2 --benchmarks=fillrandom,overwrite,stats --use_direct_io_for_flush_and_compaction \
		--key_size=20 --value_size=800 --num=300000000 --max_background_jobs=$max_bk_jobs --disable_wal
elif [ $1 -eq 6 ]
then
	echo "(Phase 2) Overwrite"
	./db_bench --fs_uri=zenfs://dev:nvme3n2 --benchmarks=overwrite,stats --use_direct_io_for_flush_and_compaction --key_size=20 --value_size=800 --num=300000000 --max_background_jobs=$max_bk_jobs --use_existing_db=true
elif [ $1 -eq 7 ]
then
	echo "420K 3M FillRandom + Overwrite"
	./db_bench --fs_uri=zenfs://dev:nvme3n2 --benchmarks=fillrandom,overwrite,stats --use_direct_io_for_flush_and_compaction \
		--key_size=20 --value_size=800 --num=3000000 --max_background_jobs=$max_bk_jobs --disable_wal
fi